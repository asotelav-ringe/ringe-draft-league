...
const blankState = () => ({ coaches: [], picks: {}, matches: [], trades: [] });
...
function Home({ state, setTab }) {
  ...
  // Última ronda con al menos un resultado reportado (la jornada de número más alto).
    ...
    <div>
      ...
      {champion && (
        <div style={{
          ...
          background:"linear-gradient(135deg, #2a0d0e 0%, #16100f 60%)",
          border:"1px solid var(--accent)", borderRadius:18, padding:"34px 24px", textAlign:"center",
          boxShadow:"0 0 0 1px var(--accent), 0 24px 60px -24px rgba(194,22,26,0.7)"
        }}>
          <div style={{ fontSize:52, lineHeight:1 }}>🏆</div>
          <div className="rl-display" style={{ color:"var(--silver-dim)", fontSize:14, letterSpacing:".22em", marginTop:10, fontWeight:600 }}>Campeón de la temporada</div>
          <div className="rl-display" style={{ fontSize:44, fontWeight:700, color:"var(--accent)", lineHeight:1.05, margin:"6px 0 4px", textShadow:"0 0 24px rgba(194,22,26,0.6)" }}>{cName(champion)}</div>
          <div style={{ color:"var(--silver)", fontSize:15 }}>{cTrainer(champion)}</div>
          ...
          {(state.picks[champion] || []).length > 0 && (
            <div style={{ marginTop:18, paddingTop:18, borderTop:"1px solid rgba(194,22,26,0.3)" }}>
              <div className="rl-display" style={{ color:"var(--silver-dim)", fontSize:11, letterSpacing:".18em", marginBottom:10, fontWeight:600 }}>Equipo campeón</div>
              <div style={{ display:"flex", flexWrap:"wrap", gap:8, justifyContent:"center" }}>
                {(state.picks[champion] || [])
                  .map(id => POOL.find(p=>p.id===id)).filter(Boolean)
                  .sort((a,b)=>b.cost-a.cost)
                  .map(p => (
                    <div key={p.id} title={`${p.name} · ${p.cost} pts`} style={{
                      display:"flex", flexDirection:"column", alignItems:"center", gap:2,
                      background:"rgba(0,0,0,0.3)", border:"1px solid rgba(194,22,26,0.35)",
                      borderRadius:10, padding:"6px 8px", minWidth:56
                    }}>
                      <Sprite id={p.id} name={p.name} size={44} />
                      <span style={{ fontSize:10, color:"var(--silver)", fontWeight:600, textAlign:"center", lineHeight:1.1 }}>{p.name}</span>
                    </div>
                  ))}
              </div>
            </div>
  ...
  return (
    <div style={{ marginTop:40, borderTop:"1px solid var(--line)", paddingTop:8 }}>
      ...
      <ul style={{ margin:"0 0 12px", paddingLeft:20 }}>
        <Li>Cada entrenador empieza con un presupuesto de <strong>100 puntos</strong> para draftear un equipo de <strong>10 Pokémon</strong> desde el Draft Board.</Li>
...
function Coaches({ state, setState, unlocked }) {
  ...
  return (
    <div>
      <SectionTitle title="Entrenadores" sub="Registra a los participantes de la liga. Cada uno tendrá 100 puntos para draftear." />
      {unlocked
        ? <div style={{
            ...
            <button onClick={add} style={btnPrimary}>+ Agregar</button>
          </div>
        : <LockedNote />}
...
// ============ DRAFT BOARD ============
function Board({ state, setState, unlocked }) {
  ...
  const pick = (pk) => {
    ...
    if (myPicks.length >= MAX_TEAM) { alert(`Cada equipo puede tener máximo ${MAX_TEAM} Pokémon.`); return; }
    ...
    setState(s => {
      ...
      if (cur.length >= MAX_TEAM) return s;
  ...
  return (
    <div>
      <SectionTitle title="Draft Board" sub="Cada entrenador tiene 100 puntos. Elige Pokémon hasta agotarlos. Un Pokémon ya drafteado no se puede repetir." />
      {!unlocked && <LockedNote />}
            ...
            <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"center" }}>
              {state.coaches.map(c => {
                ...
                return (
                  ...
                  }}>{c.team} <span style={{opacity:.7, fontWeight:500}}>· {100-sp}p</span></button>
                );
              })}
            </div>
            {activeCoach && (
              <div style={{ marginTop:12, display:"flex", gap:10, alignItems:"center" }}>
                ...
                <div style={{ fontWeight:800, fontSize:14, whiteSpace:"nowrap" }}>
                  ...
                  <span style={{ color: myPicks.length>=MAX_TEAM?"var(--accent)":"var(--ink-dim)", fontWeight:700, marginLeft:10 }}>{myPicks.length}/{MAX_TEAM} pkmn</span>
                </div>
              </div>
            )}
...
function Teams({ state }) {
  ...
  return (
    <div>
      <SectionTitle title="Equipos" sub="Así quedaron los rosters de cada entrenador." />
            ...
            <div key={c.id} style={{ background:"var(--panel)", border:"1px solid var(--line)", borderRadius:16, overflow:"hidden" }}>
              ...
              {mons.length===0
                ...
                : <div style={{ padding:10 }}>
                    {mons.map(m => (
                      <div key={m.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"5px 8px", borderRadius:8 }}>
...
function Matchups({ state, setState, unlocked }) {
  const cName = (id) => state.coaches.find(c=>c.id===id)?.team || "—";
  const cTrainer = (id) => state.coaches.find(c=>c.id===id)?.trainer || "";
  ...
  const setScore = (mid, side, val) => {
    setState(s => ({
      ...
      matches: s.matches.map(m => {
        ...
        if (nowDone && !(m.sa >= need || m.sb >= need)) next.ts = Date.now();
...
// ============ INTERCAMBIOS ============
function Trades({ state, setState, unlocked }) {
  const cName = (id) => state.coaches.find(c=>c.id===id)?.team || "—";
  const cTrainer = (id) => state.coaches.find(c=>c.id===id)?.trainer || "";
  ...
  const toggle = (list, setList, id) => {
    ...
    else alert("Máximo 2 Pokémon por entrenador en cada intercambio.");
  };
        ...
        <div
          ...
          <div onClick={e=>e.stopPropagation()} style={{
            ...
            <p style={{ color:"var(--ink-dim)", fontSize:13, margin:"0 0 16px", lineHeight:1.5 }}>
              Introduce la contraseña para poder editar la liga.
            </p>
            <input
              type="password"
              autoFocus
            ...
            />
            {pwError && <div style={{ color:"var(--accent-soft)", fontSize:12, marginTop:8, fontWeight:600 }}>Contraseña incorrecta.</div>}
        ...
        <header style={{ marginBottom:26 }}>
          <div style={{
            ...
            <div style={{
              ...
              }}>{unlocked?"🔓 Editando":"🔒 Bloqueado"}</button>
            </div>
          </div>
          <div className="rl-display" style={{ textAlign:"center", color:"var(--silver-dim)", fontSize:13, marginTop:12, fontWeight:600, letterSpacing:".22em" }}>
            Pokémon VGC · Liga privada «Matachanchos»
          </div>
        </header>
        <Nav tab={tab} setTab={setTab} />
        {tab==="home" && <Home state={state} setTab={setTab} />}
        {tab==="board" && <Board state={state} setState={guardedSetState} unlocked={unlocked} />}
        {tab==="coaches" && <Coaches state={state} setState={guardedSetState} unlocked={unlocked} />}
        {tab==="teams" && <Teams state={state} />}
        {tab==="matchups" && <Matchups state={state} setState={guardedSetState} unlocked={unlocked} />}
        {tab==="trades" && <Trades state={state} setState={guardedSetState} unlocked={unlocked} />}
      </div>
    </div>
  );
}
