...
const blankState = () => ({ coaches: [], picks: {}, matches: [], trades: [], settings: { ...DEFAULT_SETTINGS } });
...
function Nav({ tab, setTab, unlocked }) {
  const tabs = [
    ...
    ["matchups", "Matchups"],
    ["trades", "Intercambios"],
    ...(unlocked ? [["settings", "Configuración"]] : []),
  ];
  return (
...
function Settings({ state, setState, unlocked }) {
  ...
  }, [s.budget, s.maxCoaches, s.maxPicks, s.maxTrade]);
  ...
  const dirty = fields.some(({ key }) => String(s[key]) !== draft[key]);
  ...
  return (
    <div style={{ minHeight:"100vh", background:"var(--bg)", color:"var(--ink)", fontFamily:"'Chakra Petch', system-ui, sans-serif", position:"relative" }}>
      ...
      <div style={{ maxWidth:980, margin:"0 auto", padding:"24px 20px 80px", position:"relative", zIndex:1 }}>
        ...
        {tab==="settings" && <Settings state={state} setState={guardedSetState} unlocked={unlocked} />}
      </div>
    </div>
  );
}
